...
% % % 
% % % legend('Original Data','Translated and rotated data', 'Recovered data', 'Recovered data pca frame')
% % % 
% % % plot3(m(1), m(2), m(3), 'r*')
...
T = [   -0.26897    0.370448    0.889058 -0.00475106
   -0.51106    0.727517   -0.457751 -0.00859242
   0.816378    0.577484  0.00635932    0.046541
          0           0           0           1];

X = [-0.062446 -0.0546231 -0.0493529]
Y = [0.0702102 0.0784551 0.0498989]

plotOrientedIsobox(X, Y, eye(4))
hold on
data_r = (inv(T)*([data ones(size(data_r,1),1)].')).';
plot3(data_r(:,1), data_r(:,2), data_r(:,3), 'm*');
data_recover = (T*data_r.').';
%subplot(2,2,1); 
plot3(data_recover(:,1), data_recover(:,2), data_recover(:,3), 'ro');
